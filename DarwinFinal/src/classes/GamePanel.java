...
public final class GamePanel extends JPanel implements KeyListener {
    ...
    private final Timer winCondition;
    ...
    private final int ENEMY_AMOUNT = 20;
    ...
    GamePanel() {
        ...
        initClasses();
        // game loop timer occurs every 10 milliseconds
        ...
        /**
         * the win condition timer plays once the win condition has been met
         */
        winCondition = new Timer(1000, (ActionEvent ae) -> {
            // stop the game loop
            gameLoop.stop();
            // congratulate player for winning
            JOptionPane.showMessageDialog(null,
                    "Trump city has been successfully defended!"
    ...
    protected void paintComponent(Graphics g) {
        ...
        if (enableHUD) {
            ...
            g2d.drawString("HORIZ: " + (int) (Math.abs(xDelta * 12))
                    + "m/s", 300, 300 + HUD_TEXT_HEIGHT * 1);
            // print drone vertical movement
            g2d.drawString("VERT: " + (int) (-yDelta * 12) + "m/s", 300,
        ...
        for (Point2D.Double i : star) {
            if (i.getX() >= xCam
                    && i.getX() <= xCam + X_CENTER * 2
                    && i.getY() >= yCam
                    && i.getY() <= yCam + Y_CENTER * 2) {
        ...
        // set color as orange for thruster
        ...
        // the brush is set to a thicker one
    ...
    public void keyReleased(KeyEvent ke
        ...
        if (ke.getKeyCode() == KeyEvent.VK_H) {
            // the HUD is toggled to the opposite of what it currently is
            enableHUD = !enableHUD;
        }
    ...
    private void initClasses() {
        ...
        textHUD = new Font(null, 0, 17);
        // array of enemies is initialized
        enemy = new Enemy[ENEMY_AMOUNT];
        // a linked list of enemies is added
        enemies = new LinkedList();
        for (int i = 0; i < enemy.length; i++) {
            // create a new enemy class for each index
            enemy[i] = new Enemy();
            // the new enemy is added to the linked list
            enemies.add(enemy[i]);
        ...
        enemyBlip = new Ellipse2D.Double[ENEMY_AMOUNT];
    ...
    private void moveDrone() {
        ...
        // check if drone has shot a bullet
        ...
        for (int i = 0; i < enemyBlip.length; i++) {
            ...
            if (enemy[i].isDead == false) {
                ...
                enemyBlip[i].setFrame(X_CENTER - drone.getX() / 10
                        + (enemy[i].getX() - enemy[i].getSize() / 2) / 10,
                        Y_CENTER - drone.getY() / 10
                        + (enemy[i].getY() - enemy[i].getSize() / 2) / 10, 5, 5);
        ...
        for (Enemy i : enemy) {
            ...
            if (i.isDead == false) {
                ...
                i.setFrame(i.getMinX(), i.getMinY() + i.vSpeed,
                        i.getSize(), i.getSize());
                // if the enemy touches the ground... 
                if (i.getMaxY() > zone.getMaxY()) {
                    // stop the game loop
                    gameLoop.stop();
                    // show game over message
                    JOptionPane.showMessageDialog(null,
                            "Trump city has been compromised!\nGame over.");
                    // exit the program
                    System.exit(0);
                }
            }
    ...
    private void checkForEnemyDeaths() {
        ...
        for (Enemy i : enemy) {
            ...
            if (laser.intersects(i)) {
                // enemy becomess dead
                i.die();
                // enemy gets removed from the linked list
                enemies.remove(i);
                ...
                if (enemiesDead == ENEMY_AMOUNT) {
                    // wave number increases
                    waveNumber++;
                    // check if the current wave is 3
                    if (waveNumber > 2) {
                        // start the win condition
                        winCondition.start();
                        // this only applies because the game is a work in progress
                    } else {
                        ...
                        for (int k = 0; k < enemy.length; k++) {
                            ...
                            enemy[k].setFrame(Math.random() * (ZONE_SIZE - enemy[k].getSize()),
    ...
    /**
     * move the game camera to the position of the drone
     */
